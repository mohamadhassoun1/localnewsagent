...
const StorePageHeader: React.FC<{ storeName: string; onLogout: () => void }> = ({ storeName, onLogout }) => (
    <div className="flex items-center justify-between mb-6">
        <div>
            <p className="text-xl font-bold text-text-dark">{storeName}</p>
            <p className="text-sm text-text-light">Items Overview</p>
        </div>
        <button 
            onClick={onLogout} 
            className="bg-gray-200 text-text-dark font-bold py-2 px-4 rounded-lg hover:bg-gray-300 transition"
        >
            Logout
        </button>
    </div>
...
const StorePage: React.FC<StorePageProps> = ({ appData, currentUser, onLogout, onAddItem, onUpdateItem, onDeleteItem }) => {
  const { stores, items, staff, accessCodes } = appData;
  const store = stores.find(s => s.code === currentUser.storeId);
  
  const currentStoreItems = useMemo(() => {
    return items.filter(item => item.storeCode === currentUser.storeId);
  }, [items, currentUser.storeId]);
                ...
                <>
                    <div className={`flex justify-between items-center mb-4 bg-gray-100 p-1 rounded-full space-x-1 transition-opacity ${showAllDates ? 'opacity-50 pointer-events-none' : ''}`}>
                        {[Status.ExpiringSoon, Status.Active, Status.Expired].map(status => (
                            <button key={status} onClick={() => setActiveFilter(status)} disabled={showAllDates} className={`w-full py-2 px-2 text-sm font-semibold rounded-full transition-all ${activeFilter === status ? 'bg-primary text-white shadow-md' : 'bg-transparent text-text-light'}`}>
                     ...
                     <div className="flex justify-end items-center space-x-4 mb-4">
                        <label htmlFor="show-all-toggle-staff" className="flex items-center cursor-pointer">
                            <span className="mr-2 text-sm font-medium text-text-light">Show All Dates</span>
                            <div className="relative">
                                <input id="show-all-toggle-staff" type="checkbox" className="sr-only peer" checked={showAllDates} onChange={() => setShowAllDates(!showAllDates)} />
  ...
  return (
    <div className="max-w-md mx-auto bg-background min-h-screen sm:shadow-2xl sm:my-4 sm:rounded-2xl pb-28 overflow-y-auto hide-scrollbar">
        <main className="p-4 sm:p-6">
            <StorePageHeader storeName={store?.name || 'Store Dashboard'} onLogout={onLogout} />
            {isSearchVisible && (
        ...
        <BottomNavBar onAddClick={handleAddNewClick} onHomeClick={handleHomeClick} onSearchClick={handleSearchClick} onAlertsClick={handleAlertsClick} onRanksClick={handleRanksClick} onAiClick={handleAiClick} isSearchActive={isSearchVisible} isAlertsActive={activeView === 'alerts'} isHomeActive={activeView === 'items'} isRanksActive={activeView === 'ranks'} isAiActive={activeView === 'ai'} />
